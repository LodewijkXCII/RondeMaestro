  ...
  <DefaultLayout>
    ...
    <main class="wrapper">
      <section class="wrapper flow">
        ...
        <p>
          De <router-link to="/">laaste etappe</router-link> gaf je XXX punten
          en daarmee behaalde je plek YYY!. Bekijk nu snel de
          <router-link to="/">uitslag</router-link>! In het
          <router-link to="/">klassement</router-link> sta je op plek UUU met
          XXX punten. Dat zijn er maar III minder dan USERNAME.
        </p>
        <p v-if="nextStage">
          De eerstvolgende etappe is #{{ nextStage.stage_nr }}
          {{ nextStage.start_city }} - {{ nextStage.finish_city }} een etappe
          van {{ nextStage.distance }}km en heeft een kenmerk van
